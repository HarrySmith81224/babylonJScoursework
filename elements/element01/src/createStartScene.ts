...
import {
  ...
  function createGround(scene: Scene, w: number, h: number, px: number) {
    ...
    const Ground = new StandardMaterial("Ground");
    Ground.diffuseTexture = new Texture("/textures/lava.png");
  ...
  export default function createStartScene(engine: Engine) {
    ...
    //shape 1 and light
    that.faceBox = createFacedBox(that.scene, -20, 2.5, 0, 0.5, 150, 150);
    ...
    that.box = createBox(that.scene, 2, 2.5, 0, 0.5, 20, 9, false); //wall 
